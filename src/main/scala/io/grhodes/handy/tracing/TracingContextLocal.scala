...
object TracingContextLocal {
  /**
    ...
    */
  type Context = Array[Option[_ <: Traceable]]
  ...
  private def set(i: Int, v: Option[_ <: Traceable]): Unit = {
    assert(i < size)
    var ctx = localCtx.get

    if (ctx == null)
      ctx = new Array[Option[_ <: Traceable]](size)
    else {
      val oldCtx = ctx
      ctx = new Array[Option[_ <: Traceable]](size)
  ...
  /**
    ...
    */
  def closed[R](fn: () => R): () => R = {
    val closure = TracingContextLocal.save()
    () => {
      val save = TracingContextLocal.save()
      TracingContextLocal.restore(closure)
      try fn()
      finally TracingContextLocal.restore(save)
    }
  }
}

/**
  * A TracingContextLocal is a [[ThreadLocal]] whose scope is flexible. The state of all Locals may
  ...
  * have to worry about having to merge two [[io.grhodes.handy.tracing.TracingContextLocal.Context Contexts]].
  *
  * Note: the implementation is optimized for situations in which save and
  * restore optimizations are dominant.
  */
final class TracingContextLocal[T <: Traceable] {
  private[this] val me = TracingContextLocal.add()
  ...
  /**
    ...
    * General usage should be via [[let]] to avoid leaks.
    */
  def set(optValue: Option[T]): Unit = TracingContextLocal.set(me, optValue)

  /**
    * Get the Local's optional value.
    */
  def apply(): Option[T] = TracingContextLocal.get(me).asInstanceOf[Option[T]]
  ...
  /**
    * Clear the Local's value. Other [[TracingContextLocal Locals]] are not modified.
    *
    * General usage should be via [[letClear]] to avoid leaks.
    */
  def clear(): Unit = TracingContextLocal.clear(me)
}
